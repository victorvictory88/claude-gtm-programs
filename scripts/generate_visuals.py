...
def make_architecture() -> None:
    ...
    draw.text((40, 68), "Repo map for GTM Programs interview portfolio", font=sub_f, fill=MUTED)
...
def make_baseline_card() -> None:
    ...
    for label, value in rows:
        ...
        for i, line in enumerate(wrap_text(draw, value, body_f, w - 120)):
            draw.text((48, y + 24 + i * 22), line, font=body_f, fill=TEXT)
        y += 24 + 22 * max(1, len(wrap_text(draw, value, body_f, w - 120))) + 12
...
def make_seller_play() -> None:
    ...
    draw.text((40, h - 48), "Fictional example for interview portfolio. Not a real customer play.", font=small_f, fill=MUTED)
...
def harness_summary() -> dict:
    ...
    try:
        ...
        # harness may exit 0 or 1; stdout should still be JSON
...
def make_eval_terminal() -> None:
    ...
    for line in lines:
        ...
        if stripped.startswith('"') and '":' in stripped:
            ...
            # refine string/number coloring for value part
            val = rest.strip()
...
def _wrap_json_display_lines(pretty: str, max_chars: int = 88) -> list[str]:
    ...
    for line in pretty.splitlines():
        if len(line) <= max_chars:
            out.append(line)
            continue
        # Prefer wrapping after ": " for long string values
        if '": "' in line:
            prefix, _, value = line.partition('": "')
            prefix = prefix + '": "'
            # value ends with optional ",
            trailer = ""
            if value.endswith('",'):
...
def make_program_plan_json() -> None:
    ...
    for line in lines:
        ...
        if stripped.startswith('"') and '":' in stripped:
            ...
            # continuation of wrapped string value
